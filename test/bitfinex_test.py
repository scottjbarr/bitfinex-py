...
class BitfinexTest(unittest.TestCase):
    ...
    def test_should_have_ticker_for_symbol(self):
        expected = self.client.url_for("foo/%s", path_arg='bar')
    ...
    def test_should_have_lendbook(self):
        ...
        url = self.client.url_for(bitfinex.PATH_LENDBOOK, path_arg='btc')
        httpretty.register_uri(httpretty.GET, url,
                           body=mock_body,
                           status=200)
    ...
    def test_should_have_lendbook_with_parameters(self):
        ...
        url = self.client.url_for(bitfinex.PATH_LENDBOOK, path_arg='btc', parameters=parameters)
        httpretty.register_uri(httpretty.GET, url,
                           body=mock_body,
                           status=200)
